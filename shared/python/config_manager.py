...
class ConfigManager:
    ...
    def get_config(self, identifier, is_json=True):
        ...
        if self.config_source == "parameter_store":
            if isinstance(identifier, str):
                ...
                raise ValueError(
                    "For parameter_store source, identifier must be a string"
                )
    ...
    def put_config(self, identifier, config_value, **kwargs):
        ...
        if self.config_source == "parameter_store":
            if isinstance(identifier, str):
                return self.put_parameter_store_config(
                    identifier, config_value, **kwargs
                )
            else:
                raise ValueError(
                    "For parameter_store source, identifier must be a string"
            ...
            if (
                ...
                return self.put_s3_config(
                    identifier["bucket"], identifier["key"], config_value
                )
            else:
                raise ValueError(
                    "For s3 source, identifier must be a dict with 'bucket' and 'key'"
