...
def remediate_security_group_issues(security_group_id, issues):
            ...
            if issue.get('Type') == 'PublicAccess':
                ...
                if not cidr:
                    failed_remediations.append({
                        'Issue': issue,
                        'Error': 'Missing CIDR information'
                    })
                    continue
    ...
    return {
        'SecurityGroupId': security_group_id,
        'Status': 'Remediated' if remediated_issues and not failed_remediations else 'PartiallyRemediated' if remediated_issues else 'Failed',
    ...
    try:
        ...
        for key, recommended_value in best_practices.items():
            ...
            if key == 'MinimumPasswordLength':
                compliance[key] = {
                    'Status': 'Compliant' if current_value and current_value >= recommended_value else 'NonCompliant',
                    'CurrentValue': current_value,
                    'RecommendedValue': recommended_value
                }
            elif key in ['RequireSymbols', 'RequireNumbers', 'RequireUppercaseCharacters', 'RequireLowercaseCharacters']:
                compliance[key] = {
        ...
        compliant_count = sum(1 for item in compliance.values() if item['Status'] == 'Compliant')
        total_checks = len(compliance)
        compliance_percentage = (compliant_count / total_checks) * 100 if total_checks > 0 else 0
        
        return {
            'Status': 'Compliant' if compliance_percentage == 100 else 'PartiallyCompliant' if compliance_percentage > 0 else 'NonCompliant',
    ...
    try:
        ...
        for trail in trails:
            ...
            trail_details.append({
                ...
                'Status': 'Healthy' if is_logging and is_multi_region and log_file_validation else 'Suboptimal'
            })
        ...
        return {
            'Status': 'Healthy' if healthy_trails else 'Suboptimal' if trails else 'NotConfigured',
...
def check_root_account_mfa():
    ...
    try:
        ...
        account_mfa_enabled = bool(response.get('SummaryMap', {}).get('AccountMFAEnabled', 0))
